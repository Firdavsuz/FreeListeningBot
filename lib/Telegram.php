...
class Telegram
{
    ...
    public function getJoinRequest($id, $channels = []): array
    {
        $i = 0;
        $button = [];
        $txt = '';
        ...
        foreach ($channels as $channel) {
            $i++;
            $infoChannel = $this->endpoint('getChat', [
                'chat_id' => $channel
            ])['result'];
            $result = $this->endpoint('getChatMember', [
                'chat_id' => $channel,
                'user_id' => $id
            ])['result']['status'];
            if ($result == 'creator' or $result == 'administrator' or $result == 'member') {
                ...
                $isJoin = false;
                $txt .= "$i) [" . $infoChannel['title'] . "](https://t.me/" . $infoChannel['username'] . ")\n";
                $button [] = ['text' => $i . ' - Kanalga obuna bo\'lish', 'url' => 'https://t.me/' . $infoChannel['username']];
            }
        }
        $bk = array_chunk($button, 1);
        $keyboard = json_encode([
            'inline_keyboard' => $bk
        ]);
        if (!$isJoin) return ['ok' => false, 'text' => $txt, 'button' => $keyboard];
        else return ['ok' => true];
    }
    ...
    public function FirstName()
    {
        ...
        if ($type == self::CHAT_MEMBER) {
            return @$this->data['chat_member']['from']['first_name'];
        }
    ...
    public function LastName()
    {
        ...
        if ($type == self::CHAT_MEMBER) {
            return @$this->data['chat_member']['from']['last_name'];
        }
     ...
     * \return the requested button as Array.
     */
    public function buildKeyboardButton($text, $web_app = [], $request_contact = false, $request_location = false)
    {
        $replyMarkup = [
            'text' => $text,
    ...
    public function getUpdateType()
    {
        ...
        if (isset($update['chat_member'])) {
            $this->update_type = self::CHAT_MEMBER;
